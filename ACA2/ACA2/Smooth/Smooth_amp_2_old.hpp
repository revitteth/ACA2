...
void smooth_amp_2_main(Mesh* mesh, size_t vid) {
	  ...
	  concurrency::array<size_t, 1> it_par(mesh->NEList[vid].size(), mesh->NEList[vid].begin(), mesh->NEList[vid].end());

	  concurrency::array_view<double, 1> coords(mesh->NEList[vid].size(), mesh->coords);
	  concurrency::array_view<size_t, 1> ENList(mesh->NEList[vid].size(), mesh->ENList);
	  concurrency::array_view<double, 1> metric(mesh->NEList[vid].size(), mesh->metric);


		concurrency::parallel_for_each(
			it_par.extent, 
			[=, &it_par](concurrency::index<1> idx) restrict(amp)
			{

		//////////////////////////////////////////////////////

			  const size_t *n = &ENList[3*idx];
