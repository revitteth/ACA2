...
#ifndef SMOOTH_tbb_HPP_
#define SMOOTH_tbb_HPP_
...
class SmoothSubTask
{
	...
	public:
		SmoothSubTask(Mesh* mesh) : mesh(mesh) {};
...
class SmoothMainTask : public tbb::task
{
	private:
		Mesh* mesh;
		size_t niter;
	...
	public:
		SmoothMainTask(Mesh* mesh, size_t niter) : mesh(mesh), niter(niter) {};

		tbb::task* execute(void)
		{
			tbb::task_list t; // Define t as the list of tasks
			set_ref_count(1); // root task
			for(size_t iter=0; iter<niter; ++iter){
				//increment_ref_count(); 
				//t.push_back(*new (task::allocate_child()) SmoothSubTask(mesh));
						//	for(size_t vid=0; vid<mesh->NNodes; ++vid){
				parallel_for(tbb::blocked_range<size_t>(0, mesh->NNodes), SmoothSubTask(mesh));
			}
			spawn_and_wait_for_all(t); // Waiting for all tasks in t
			return NULL;
		}
};

void smooth_tbb(Mesh* mesh, size_t niter){
  // For the specified number of iterations, loop over all mesh vertices.
//	tbb::task_scheduler_init init(CoreUsageController::getCoreUsage());
	SmoothMainTask &taskRoot = *new (tbb::task::allocate_root()) SmoothMainTask(mesh, niter);
	tbb::task::spawn_root_and_wait(taskRoot);
